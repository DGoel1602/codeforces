...
export async function fetchSubmissions(
	...
	const submissions = await request("user.status", params, credentials);
	if (!submissions.every(isSubmission)) {
		throw new Error(
			"Codeforces user.status returned an invalid submission",
		);
	}
...
function isSubmission(value: unknown): value is Submission {
	const submission = value as Partial<Submission> | null;
	if (
		!submission ||
	...
	return (
		...
		(problem.rating === undefined ||
			Number.isSafeInteger(problem.rating)) &&
		[submission.contestId, problem.contestId].every(
...
export async function fetchRating(handle: string): Promise<number | null> {
	...
	if (user.rating !== undefined && !Number.isSafeInteger(user.rating)) {
		throw new Error(
			`Codeforces returned an invalid rating for "${handle}"`,
		);
	}
...
async function request(
	...
	if (body?.status === "FAILED") {
		throw new Error(`Codeforces: ${body.comment}`);
	}
...
function apiUrl(
	...
	const payload = `${prefix}/${method}?${url.searchParams}#${credentials.secret}`;
	const signature = createHash("sha512").update(payload).digest("hex");
